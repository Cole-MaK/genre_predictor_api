...
class PredictPipeline:
    ...
    def predict(self, features):
        try:
            ...
            genre_dict = 'src/artifact/genre_dict.pkl' 
            model=load_object(file_path=model_path)
            preprocessor=load_object(file_path=preprocessor_path)
            genres = load_object(file_path=genre_dict)
            data_preprocess = preprocessor.transform(features)
            result = model.predict(data_preprocess)
            prediction = result.astype(int).tolist()[0]
            prediction = genres[prediction]
            return prediction
